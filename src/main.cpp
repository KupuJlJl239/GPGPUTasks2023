...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        // OCL_SAFE_CALL(clGetPlatformInfo(platform, 239, 0, nullptr, &platformNameSize));   //   ==> CL_INVALID_VALUE
        ...
        std::cout << "    Number of OpenCL platform devices: " << devicesCount << std::endl;
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            if(deviceType == CL_DEVICE_TYPE_CPU)        deviceTypeName = "CPU";
            else if(deviceType == CL_DEVICE_TYPE_GPU)   deviceTypeName = "GPU";
            else                                        deviceTypeName = "something strange";

            std::cout << "        Device type: " << deviceTypeName << std::endl;

            // размер памяти
            cl_ulong memsize;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(cl_ulong), &memsize, nullptr));
            std::cout << "        Device memory: " << (memsize >> 10) << " KB"<< std::endl;  // у меня это 32 КБ, поэтому я сделал не в MB
            ...
            std::cout << "        OpenCL version: " << version.data() << std::endl;
